...
BEGIN
  ...
  IF EXISTS (
    ...
    FROM public.sesiones se
    WHERE se.id = ANY(v_session_ids)
      AND NOT (se.equipo_id = ANY(v_team_ids))
  ) THEN
    RAISE EXCEPTION 'session requires its selected team';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE sd.sesion_id = ANY(v_session_ids)
  ...
  IF EXISTS (
    ...
    WHERE se.sesion_id = ANY(v_session_ids)
  ...
  FOREACH v_source_team_id IN ARRAY v_team_ids LOOP
    ...
    INSERT INTO public.equipos (
      ...
      sede_id,
      workspace_id,
      entrenador_principal_id,
      entrenador_adjunto_id
    )
    VALUES (
      v_source_team.nombre,
      v_source_team.categoria,
      v_destination_sede.id,
      p_workspace_id,
      NULL,
      NULL
    )
  ...
  FOREACH v_source_session_id IN ARRAY v_session_ids LOOP
    SELECT se.*
      INTO v_source_session
    FROM public.sesiones se
    WHERE se.id = v_source_session_id;
    ...
    INSERT INTO public.sesiones (
      ...
      feedback_post_entreno,
      estado
    )
    VALUES (
      v_destination_team_id,
      CASE
        WHEN v_source_session.entrenador_id = ANY(v_trainer_ids)
          THEN v_source_session.entrenador_id
        ELSE NULL
      END,
      v_source_session.fecha,
      v_source_session.hora_inicio,
      v_source_session.duracion_estimada,
      v_source_session.microciclo,
      v_source_session.periodo_temporada,
      v_source_session.objetivo_sesion,
      v_source_session.observaciones_previas,
      NULL,
      v_source_session.estado
    )
