...
export function useParametros(categoria: string, workspaceId: string | null) {
  const query = useQuery<ParametroSistema[]>(
    () =>
      workspaceId
        ? fetchParametrosByCategoria(categoria, workspaceId)
        : Promise.resolve({ data: [], error: null }),
    [categoria, workspaceId],
  );
  ...
  const updateMutation = useMutation<
    ParametroSistema,
    { id: string; input: ParametroSistemaUpdateInput }
  ...
  const createOne = useCallback(
    async (input: ParametroSistemaCreateInput) => {
  ...
  const updateOne = useCallback(
    async (id: string, input: ParametroSistemaUpdateInput) => {
